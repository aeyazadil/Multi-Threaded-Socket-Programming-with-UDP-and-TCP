...
/* Client: connects via TCP, negotiates UDP, sends data. */
#include "socketUtils.h"
char serv_addr_input[20] = "127.0.0.1";

// Send a UDP message and read server response
int udp_communicate(int received_port_number)
{
	...
	serv_addr.sin_addr.s_addr = inet_addr(serv_addr_input); // Server IP
	serv_addr.sin_port = htons(received_port_number);		// UDP port
...
// Entry point: connect via TCP, get UDP port, then use UDP
int main(int argc, char **argv)
{
	...
	*/
	...
	int received_port_number;
	sscanf(Phase1Message2->message, "%d", &received_port_number);
